...
def bounding_boxes_random_remove(dataFrame, filename, save=True, plot=False):
    print('filename', filename)
    annotation_file = 'rand_annotation.csv'  # Edit this line with your own annotation file

    data = dataFrame  # Parses inkml file

    cols = ['x_min', 'x_max', 'y_min', 'y_max', 'class']  # Columns of bboxs dataframe
    bboxs = pd.DataFrame(
        columns=cols)  # Dataframe containing all coordinates of all bounding boxes, plus the class for each bounding box
    ...
    for k in group_id:  # For each unique trace group of the inkml file...
        ...
        bboxs = bboxs.append({'x_min': group_x[0], 'x_max': group_x[len(group_x) - 1], 'y_min': group_y[0],
                              'y_max': group_y[len(group_y) - 1], 'class': label},
                             ignore_index=True)  # Save minimum and maximum [x, y] coordinates to the bboxs dataframe
        # Add +1 to the maximum coordinates and -1 to the minimum to get

        x_coord = [group_x[0], group_x[len(group_x) - 1], group_x[len(group_x) - 1], group_x[0], group_x[
            0]]  # Contain all x coordinates of a bounding box, plus the first one repeated to be able to draw a rectangle over the figure
        y_coord = [group_y[len(group_y) - 1], group_y[len(group_y) - 1], group_y[0], group_y[0], group_y[len(
            group_y) - 1]]  # Contain all x coordinates of a bounding box, plus the first one repeated to be able to draw a rectangle over the figure
    ...
    pboxs['class'] = bboxs[
        ['class']].copy()  # Copy the labels from the original bboxs dataframe, as they will not change later

    if save:  # If save = True, append the bounding box pixel coordinates to the specified annotation file

        pboxs.insert(0, 'filename',
                     'path/' + filename)  # Insert a new column to pboxs in position 0, containing the processed file name

        cols = ['filename', 'x_min', 'y_min', 'x_max', 'y_max', 'class']  # csv file columns
        pboxs.to_csv(annotation_file, columns=cols, header=False, index=False,
                     mode='a')  # Add data to the specified annotation file
